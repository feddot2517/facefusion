...
STATES : Union[StateSet, ProcessorState] =\
{
    ...
    'api': {} #type:ignore[typeddict-item]  # Добавляем новое состояние для API
}
...
def init_item(key : Union[StateKey, ProcessorStateKey], value : Any) -> None:
    ...
    STATES['api'][key] = value #type:ignore  # Инициализируем для API
...
def sync_item(key : Union[StateKey, ProcessorStateKey]) -> None:
    ...
    STATES['api'][key] = STATES.get('ui').get(key) #type:ignore  # Синхронизируем API
